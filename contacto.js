  ...
  form.addEventListener('submit', async function(e){
    ...
    try {
      ...
      await addDoc(collection(db, 'mensajes_contacto'), {
        nombre: nombre.value.trim().slice(0, 100),
        correo: correo.value.trim().slice(0, 200),
        mensaje: mensaje.value.trim().slice(0, 2000),
        leido: false,
      ...
      showStatus('ok', '¡Gracias, ' + primerNombre + '! Tu mensaje llegó al administrador. Te responderemos a ' + correoEnviado + ' muy pronto.');

      form.reset();
      [nombre, correo, mensaje].forEach(f => f.closest('.field').classList.remove('field-valid', 'field-invalid'));
    } catch (err) {
      console.error('[contacto] Error al enviar mensaje:', err);
      showStatus('error', 'No se pudo enviar tu mensaje. Intenta de nuevo en unos minutos.');
    } finally {
