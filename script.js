...
// === Script extraído de index.html ===
        ...
        function showSection(sectionId) {
            const sections = ['view-home', 'view-about', 'view-courses', 'view-contact', 'view-register', 'view-login', 'view-dashboard', 'view-course-content', 'view-intranet'];
...
Párrafo 2: Una frase sincera de aliento para que confíe en su propio aprendizaje en la escuela (máximo 2 líneas).`;
        ...
        // ── Renderizar mini-catálogo en la intranet ──
        function renderIntranetCatalog() {
            ...
            grid.innerHTML = coursesData.slice(0, 6).map(c => `
                <div class="bg-white border border-slate-150 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all group cursor-pointer"
                     onclick="openCourseContent('${c.id}')">
                    <div class="${c.bgColor} h-28 flex items-center justify-center text-white text-4xl opacity-90 group-hover:opacity-100 transition-all">
                    ...
                    <div class="p-4 space-y-1.5">
                        ...
                        <div class="flex items-center justify-between pt-1">
                            <span class="text-[10px] text-slate-400">${c.lessons} lecciones · ${c.duration}</span>
                            <span class="text-[10px] font-bold ${c.price === 'Gratuito' ? 'text-emerald-600' : 'text-brandLightBlue'}">${c.price}</span>
                        </div>
                    </div>
                </div>
            `).join('');
